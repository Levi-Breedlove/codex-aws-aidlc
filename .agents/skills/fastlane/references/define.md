...
- `Accept this recommendation.` applies only to the current decision when its recommendation is complete and needs no detail. It never resolves a factual, stale, changed, or multi-question legacy card. The exact legacy phrase `Accept all recommendations.` and matching `R-*` input remain hidden compatibility aliases and are never displayed.
