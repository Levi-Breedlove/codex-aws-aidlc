...
- Owner-facing first-screen summaries are generated, non-authoritative views bounded by the exact `FASTLANE:DOCUMENT_SUMMARY` markers. Canonical parsers, routing, and every gate, design, task, evidence, and authority digest exclude the complete marked block. Never hand-edit an Owner Decision Brief into project state.
...
- Use balanced, labeled `<details>` only for appendices or exact records. Every summary and opening sentence must explain the records to an owner; collapsed content may not hide Codex instructions, parser rules, migrations, digests, routing, or AWS operating procedure. Never collapse the current action, material risk, project command, project diagram, readiness summary, approval boundary, or formal receipt.
- Raw file length is not the readability measure. Keep visible nonblank lines at or below PRD 330, TASKS 65, VERIFY 90, RUNBOOK 220, and BUGFIX 90 while retaining complete canonical records.
