...
- Owner-facing summaries are derived from canonical records. Never hand-edit an Owner Decision Brief into project state.
- Use `<details>` only for appendices or exact records. Never collapse the current action, readiness summary, approval boundary, or formal receipt.
